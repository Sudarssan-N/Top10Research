...
def hello():
    return "Hello from P06 scaffold — ready to research!"
...
# P6 synthetic (hidden from "long context" + degradation label)
def make_synthetic_batch(batch_size: int = 4, hidden_size: int = 2048, seed: int = 42):
    import torch
    torch.manual_seed(seed)
    hidden = torch.randn(batch_size, hidden_size)
    # Higher degradation for "middle" context simulation
    target = (torch.rand(batch_size) > 0.6).float()
