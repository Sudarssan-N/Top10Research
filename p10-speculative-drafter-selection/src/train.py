...
from .core import DrafterSelector  # P10 specific
import os
...
def train(config: ExperimentConfig, train_loader: DataLoader, model: torch.nn.Module, device: str):
    ...
    criterion = torch.nn.CrossEntropyLoss()  # multi-class drafter selection for P10
    ...
    for epoch in range(config.num_epochs):
        ...
        for step, batch in enumerate(pbar):
            ...
            out = model(hidden)
            loss = criterion(out["drafter_logits"], target)
