# How to Run — P4: Test-Time Compute vs. Confidence Calibration + Corrective Probe
...
- [x] CalibrationProbe (hidden -> calibrated prob) + smoke with compute_ece before/after demo
- [x] Training (BCE) + synthetic data
- [ ] Budgeted generation collection (different max_new_tokens / #reasoning steps)
- [ ] Raw confidence extraction from model + true label (correct answer match)
- [ ] Train probe + post-hoc calibration (probe + temp scaling / isotonic)
- [ ] Full analysis: ECE, Brier, reliability diagrams vs test-time budget
- [ ] Show probe can fix degradation from over-reasoning
- [ ] Plots/tables + comparison to arXiv:2508.15050 findings
- [ ] Low-FPR operating point results (relevant to selection)

## References (key papers to cite / implement against)

arXiv:2508.15050 (over-reasoning impairs calibration)
