#!/usr/bin/env python3
"""
Main entry point for P9: Token-Efficient Agent Memory with Bounded Reasoning Cost
"""
import argparse
import os
import sys
...
from src.core import MemoryPolicyProbe as ProblemProbe  # P9 specific
...
def main():
    ...
    if args.smoke_test:
        ...
        dummy_items = [torch.randn(1, 2048) for _ in range(5)]
        out = model(dummy_items[0])
        print("Probe output keys:", list(out.keys()))
        selected = model.select_memory(dummy_items, budget_tokens=200)
        print(f"Demo selected memory indices: {selected}")
